    ...
    <p><b>Description:</b> <?= $task['discription'] ?></p> 
      ...
      <p><b>Status: </b>
      	<form action="/task/updateStatus" method="POST">
            ...
            <option value="<?= $status[1]; ?>" <?php if ($status[1] === $task['status']) { echo ' selected="selected"'; } ?>><?= $status[1]; ?></option>
            <option value="<?= $status[2]; ?>" <?php if ($status[2] === $task['status']) { echo ' selected="selected"'; } ?>><?= $status[2]; ?></option>
                <?php endforeach; ?>
          </select>
