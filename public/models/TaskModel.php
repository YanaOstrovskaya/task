...
class TaskModel extends Model
{
    ...
    public static function all()
    {
        $res = self::$pdo->query('SELECT tasks.id, tasks.name, tasks.discription, tasks.status, types.name
        	AS type, priorities.name
        	AS priority, files.path_files
        	FROM tasks
    ...
    public static function add($data)
    {
        extract($data);
        //var_dump($data[0]);
        $st = self::$pdo->prepare("INSERT INTO tasks (name, discription, type_id, priority_id)
        	VALUES(:name, :discription, :type_id, :priority_id)");
